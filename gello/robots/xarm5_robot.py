...
class XArm5Robot(Robot):
    """XArm5 Robot controller.
    
    Note: XArm5 uses joints 1, 2, 4, 6, 7 from the XArm7 configuration.
    This corresponds to array indices 0, 1, 3, 5, 6 in the servo_angle array.
    """
    GRIPPER_OPEN = 800
    GRIPPER_CLOSE = 0
    #  MAX_DELTA = 0.2
    ...
    def _update_last_state(self) -> RobotState:
        with self.last_state_lock:
            ...
            # For xArm5, map joints 1,2,4,6,7 from xArm7 (indices 0,1,3,5,6)
            servo_angle = np.array(servo_angle)
            xarm5_joints = np.array([servo_angle[0], servo_angle[1], servo_angle[3], servo_angle[5], servo_angle[6]])
    ...
    def _set_position(
        ...
        if self.robot is None:
            return
        # For xArm5, map 5 joints to xArm7 positions (joints 1,2,4,6,7 -> indices 0,1,3,5,6)
        # Create full 7-joint array with current positions, then update the 5 active joints
        code, current_servo_angle = self.robot.get_servo_angle(is_radian=True)
        if code != 0:
            print(f"Error getting current servo angles: {code}")
            return
            
        # Ensure we have exactly 5 joints for xArm5
        joints = joints[:5]
        
        # Update only the active joints (indices 0,1,3,5,6)
        full_joints = np.array(current_servo_angle)
        full_joints[0] = joints[0]  # joint 1
        full_joints[1] = joints[1]  # joint 2
        full_joints[3] = joints[2]  # joint 4
        full_joints[5] = joints[3]  # joint 6
        full_joints[6] = joints[4]  # joint 7
        
        ret = self.robot.set_servo_angle_j(full_joints, wait=False, is_radian=True)
        if ret in [1, 9]:
            self._clear_error_states()
